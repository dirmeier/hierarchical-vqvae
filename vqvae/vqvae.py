...
class Encoder(nn.Module):
  ...
  def __call__(self, inputs, is_training):
    block_ctor = (
      _ConvNextBlock if self.model_block_name == "convnext" else _ResNetBlock
    )
    hidden = inputs
    hidden = nn.Conv(
      self.n_channels,
    ...
    hidden = block_ctor(self.n_out_channels, self.n_layers)(hidden, is_training)
    return hidden
...
class VQVAE(nn.Module):
  ...
  def setup(self):
    ...
    self._encoders = [
      Encoder(**asdict(self.encoder_configs[i])) for i in range(self.n_scaling)
    ]
    self._decoders = [
  ...
  def __call__(self, inputs, is_training):
    ...
    for idx in list(reversed(range(self.n_scaling))):
      quantizable_input = [encs[idx], decs[-1]] if len(decs) else [encs[idx]]
      ...
      decoded = self._decode(idx, vq_res["quantized"], is_training, *quants)
      vq_loss += vq_res["loss"]
